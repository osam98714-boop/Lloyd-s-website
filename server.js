...
app.get('/api/tiktok/auto', validateApiKey, async (req, res) => {
    ...
    try {
        if (isUrl) {
            ...
            const videos = data.data.videos.map(v => ({ title: v.title, videoUrl: v.hdplay || v.play, author: v.author?.nickname, stats: { likes: v.digg_count, plays: v.play_count } }));
...
app.get('/api/social/download', validateApiKey, async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, title, downloadUrl, platform: isFacebook ? 'facebook' : 'instagram' });
    } catch (err) {
        console.error('[Social] Error:', err.message);
        res.status(500).json({ success: false, error: 'فشل تحميل الفيديو' });
    }
});
...
// التعديل هنا: مسار التحقق من الحالة
